...
def cucumber_reports(parser, xml_parent, data):
    """yaml: cucumber_reports
    Publish cucumber html reports.  Requires the Jenkins Cucumber Reports Plugin.
    <https://wiki.jenkins-ci.org/display/JENKINS/Cucumber+Reports+Plugin`_

    :arg str jsonReportDirectory: The path relative to the workspace of the json reports \
    generated by cucumber-jvm e.g. target - leave empty to let the plugin find them \
    automagically (default: '')
    
    :arg str pluginUrlPath: The path to the jenkins user content url e.g. \
    http://host:port[/jenkins/]plugin - leave empty if jenkins url root is host:port \
    (default: '')
    
    :arg bool skippedFails: Tick this if you want skipped steps to cause the build to \
    fail (default: false)

    :arg bool undefinedFails: Tick this if you want pending steps to cause the build to \
    fail (default: false)
    
    :arg bool noFlashCharts: Tick this if you want to use javascript charts instead of \
    flash charts (default: false)
    ...
    """
    logger = logging.getLogger(__name__)
    cr = XML.SubElement(xml_parent, 'net.masterthought.jenkins.CucumberReportPublisher')
    XML.SubElement(cr, 'jsonReportDirectory').text = data.get('jsonReportDirectory', '')
    XML.SubElement(cr, 'pluginUrlPath').text = data.get('pluginUrlPath', '')
    XML.SubElement(cr, 'skippedFails').text = \
    	str(data.get('skippedFails', False)).lower()
    XML.SubElement(cr, 'undefinedFails').text = \
    	str(data.get('undefinedFails', False)).lower()
    XML.SubElement(cr, 'noFlashCharts').text = \
    	str(data.get('noFlashCharts', False)).lower()
